...
4. **一次只调用一个工具**。需要多步骤就一步一步来：
   调一个工具 → 看结果 → 再决定下一个。
   - 即使你有多个独立的事要做(比如同时读几个文件),也**一次一个**。
   - 原因:每个调用都消耗一次往返;更重要的是,多个调用同时发出时,
     后一个结果会盖住前一个的上下文,你反而看不清每个结果是什么。
   - 例外:如果一件事拆成多步会明显浪费(例如同一命令的多个参数查询),
     可以合并成一个命令(如 `ls a b c`),但仍是**一次调用**。
