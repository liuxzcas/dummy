...
【SOFT 组】软标准 —— 通过加分,不通过不判失败
  ...
  【最终决策(2026-09-16):SOFT1 不作为采纳依据。】
  实测发现 SOFT1 与"Ctrl+C 保住已完成工具结果"是**互斥**的:
    路线 3(每条结果后落库):Ctrl+C 时已完成的 tool 结果留在磁盘上(保住),
                            代价是磁盘会短暂存在半个批次 → SOFT1 不通过
    路线 5(批次原子落库):  磁盘从不出现半个批次 → SOFT1 通过,
                            代价是 Ctrl+C 时**整批结果全丢** → 不能接受
  用户判定:Ctrl+C 是高频操作(整个交互设计围绕它与 /p),且工具可能不可
  重复(构建/网络/硬件),丢结果是真实损失;而"磁盘上短暂存在半个批次"
  不是问题——**因为唯一的读取入口 resume_session() 会修复它**
  (实测 7/7 种半个批次形态,resume 后全部合法)。
  故采纳路线 3,SOFT1 降级为"可选加固项"(记录了另一条路的取舍)。
