...
#   ui.py 在 **import 时**就读 DUMMY_UI 环境变量决定用哪个渲染器
#   (见 ui.py 的 _make_ui())。如果 load_dotenv() 放在 main() 里
#   (原来的位置在第 191 行),那么 import ui 时 .env 还没加载 ——
#   .env 里的 DUMMY_UI / DUMMY_COLOR 等**全部失效**。
#   实测复现:import ui 后是 SilentRenderer,load_dotenv 再 reload
#   才变成 LineRenderer。
load_dotenv()
...
def main():
    ...
    # ===========================================================
    # Step 0: .env 加载已在模块顶部完成(必须在 import ui 之前,
    #         否则 DUMMY_UI 等变量读不到 —— 见文件头注释)
    # ===========================================================
