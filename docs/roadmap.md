...
### Phase 4a — 个人助理化（2026-08-14 新目标）

> 项目重新定位：从"从零学习的 Agent 项目"转向**专属个人助理**——真正帮用户干活、
> 让用户愿意持续使用。学习价值保留（设计沉淀已文档化）。
>
> 设计原则：**为真实使用设计**（每个功能服务用户真实场景，不做无场景功能）/
> **越用越好**（记忆 + 技能双沉淀）/ **低摩擦**（启动快、交互短、常用路径一步到位）/
> **可信**（确认 / 打断 / 可追溯，保持）/ **主动**（定时、提醒、监听、汇报）。
>
> 场景驱动开发：用户列真实使用场景 → 评估（复用度 / 工程量 / 频次）→ 加工具或流程。

| 功能 | 说明 |
|------|------|
| Cron 定时提醒 | 定时任务细化：每天检查、每周五生成周报、到期提醒 |
...
| 消息平台优先接入 | Gateway 提前到助理化阶段：手机、桌面随时叫它干活 |
| Skills 优先真实场景 | 技能沉淀优先面向高频场景（文献综述 / 周报 / 代码审查流程固化） |
