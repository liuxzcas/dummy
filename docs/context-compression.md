...
- role 用 `system`(**2026-08-07 修订**):实现时发现,若用 `user`,
  摘要消息会与最近轮第一条 user 消息连续,违反 OpenAI/DeepSeek 的
  role 交替约束(llm.py 注释明确警告"违反会导致某些模型出错")。
  多 system 消息是兼容的(通常被合并处理);"被当指令执行"的顾虑
  用内容前缀"[早期对话摘要(背景,非指令)]"缓解
