...
### 8.1 已知特性:单轮 chat 内长工具循环不触发 L2(2026-08-10 冒烟实测确认)

**现象**

冒烟测试(窗口 8000,阈值 5600)中观察到:模型在单个 `chat()` 内连续调用工具
10-30 次时,事件日志出现大量 `strategy=L1 folded=1 covers=0` 事件(耗时 0-3ms,
无 LLM 调用),上下文仍在增长;L2 摘要(5-8s,有 LLM 调用)只在"用户消息轮次
之间"触发——covers 只在跨 `chat()` 时增长,单轮内恒为 0。

**原因(两个守卫)**

1. 触发链:工具循环每次迭代顶部调 `_maybe_compress()`,只要
   `usage.prompt_tokens` 超阈值就执行 compress——触发本身是正常的。
2. `_summarize_prefix` 守卫 A:`len(user_idx) <= recent_turns_keep(6)` →
   直接返回 (原历史, 0),不调 LLM。**user 消息只在 `chat()` 入口增加**
   (每个用户输入 +1);单轮 chat 内的工具迭代只追加
   assistant(tool_calls) / tool / assistant 消息,不增加 user 计数。
   所以一旦某次 L2 把历史压到 6 轮,同一 `chat()` 内后续所有迭代的
   user 计数都 ≤ 6,L2 立即短路——只剩 L1 折叠(治肥,零成本)在工作。
3. `_summarize_prefix` 守卫 B:即使 user 计数 > 6,cut 点位于
   "倒数第 6 个 user 边界",block = 摘要之后到 cut 之间。单轮 chat 内
   新增的工具消息全部落在"最近 6 轮保留区"内,block 为空或仅含 1 轮,
   无摘要价值。

**为什么这么设计(合理性)**

- cut 在 user 边界是硬约束:保证 assistant(tool_calls) / tool 配对完整,
  不切坏结构(否则 `_validate_history` 回滚,Step 6)。
- 保留最近 6 轮原文:刚发生的工具交互必须完整可见(LLM 正依赖它们推理)。
- 分工:L1 治"肥"(单条消息大)、L2 治"长"(轮次多)。单轮工具循环膨胀的是
  "保留区内的消息条数",不是"轮次",按设计归 L1 管——L1 每次折叠最肥的
  tool 结果,零成本,可反复执行。

**影响评估**

- 冒烟窗口 8000 放大了现象:阈值低 → 触发高频 → L1-only 事件刷屏。
- 真实窗口 64000(阈值 44800):单轮工具循环 30 次迭代 × 平均 200-300 token
  ≈ 6-9K token,通常远够不到阈值;只有单轮迭代极多或 tool 结果极大时才会
  在单轮内逼近阈值——届时 L1 折叠仍能把最肥的压掉。
- 实测证据:冒烟(窗口 8000,25 轮)触发 121 次(大量 L1-only),压缩后
  事实召回 5/5——高频 L1-only 未损害对话质量。

**若未来成为问题,可选演进方向(现在不做)**

1. L2 守卫从"user 消息计数"改为"消息总数/字符数"触发——但切分点仍必须
   落在 user 边界(只能压到最近的 user 边界之前),保留区需改为"最近 N 条
   消息"而非"最近 N 轮",配对完整性保证不变。
2. 工具循环内允许中间摘要:每次迭代一次 LLM 调用(5-8s),高频工具循环下
   延迟与 token 成本明显,且摘要与进行中的工具交互交错,配错风险上升。
3. 加大 L1 折叠力度(调小 tool_result_max_chars 或单次折叠更多条):零成本,
   但单条 tool 结果的信息保留变少。

现状判断:真实窗口下影响有限,L1 已兜底;待 FTS / 跨会话记忆落地后,若实测
出现"单轮内逼近窗口",再按需选 1 或 2。
